...
## Rules
- All mechanical work goes through the `${CLAUDE_PLUGIN_ROOT}/scripts/adlc` script — do not reimplement key generation,
  state, or Jira calls by hand.
- Jira vs local is automatic (`${CLAUDE_PLUGIN_ROOT}/scripts/adlc jira mode`). Never echo secrets; creds come from env only.
...
## Workflow
1. Detect mode: `${CLAUDE_PLUGIN_ROOT}/scripts/adlc jira mode` → `jira` or `local`.
2. Get a KEY + seed the run:
   - **local:** `${CLAUDE_PLUGIN_ROOT}/scripts/adlc init "<request>"` prints a new KEY (e.g. `ADLC-001`) and creates
     `docs/adlc/<KEY>/` with `state.md` + `ticket.md`.
   - **jira:** create/pick the issue → `${CLAUDE_PLUGIN_ROOT}/scripts/adlc jira create "<summary>" "<description>"` (prints the
     KEY) or `${CLAUDE_PLUGIN_ROOT}/scripts/adlc jira pick` (choose one), then `${CLAUDE_PLUGIN_ROOT}/scripts/adlc init "<request>" <KEY>` to seed the
     local run dir under the real key.
3. Edit `docs/adlc/<KEY>/ticket.md`: fill the Description and 3–6 acceptance criteria.
4. Record state: `${CLAUDE_PLUGIN_ROOT}/scripts/adlc set-state <KEY> jira_mode <mode>` and
   `${CLAUDE_PLUGIN_ROOT}/scripts/adlc set-state <KEY> current_stage spec`.
